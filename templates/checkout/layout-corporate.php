<?php
/**
 * Layout "Corporativo" do checkout GVN (institucional grafite: seções sublinhadas + resumo sóbrio).
 *
 * Modelo selecionável via Configurações > GVN Checkout > Layout
 * ou pelo shortcode [gvn-checkout layout="corporate"].
 *
 ...
 *
 * A seleção de pagamento usa abas segmentadas (uma por gateway ativo) com
 * painel único de campos — reaproveita o JS base, sem JS exclusivo.
 * A ordem visual é feita via CSS grid; a ordem do DOM segue a do layout
 * clássico para preservar a sequência dos hooks.
 *
...
$corporate_heading  = '' !== trim( (string) $title_text ) ? $title_text : __( 'Dados do participante', 'gvn-checkout' );
$corporate_brand    = trim( (string) $header_text );
if ( function_exists( 'mb_substr' ) && function_exists( 'mb_strtoupper' ) ) {
    $corporate_initial = '' !== $corporate_brand ? mb_strtoupper( mb_substr( $corporate_brand, 0, 1 ) ) : 'G';
} else {
    $corporate_initial = '' !== $corporate_brand ? strtoupper( substr( $corporate_brand, 0, 1 ) ) : 'G';
}
$privacy_url        = function_exists( 'get_privacy_policy_url' ) ? get_privacy_policy_url() : '';
$terms_page_id      = function_exists( 'wc_get_page_id' ) ? (int) wc_get_page_id( 'terms' ) : 0;
$terms_url          = ( $terms_page_id > 0 && function_exists( 'get_permalink' ) ) ? get_permalink( $terms_page_id ) : '';
?>

<div class="gvn-checkout gvn-layout-corporate" id="gvn-checkout" data-layout="corporate">

    <?php do_action( 'woocommerce_before_checkout_form', $checkout ); ?>

    <header class="gvn-corporate__header">
        <div class="gvn-corporate__header-inner">
            <div class="gvn-corporate__brand">
                <span class="gvn-corporate__monogram" aria-hidden="true"><?php echo esc_html( $corporate_initial ); ?></span>
                <span class="gvn-corporate__brand-text">
                    <span class="gvn-corporate__brand-name"><?php echo esc_html( $header_text ); ?></span>
                    <?php if ( '' !== trim( (string) $subtitle_text ) ) : ?>
                        <span class="gvn-corporate__brand-sub"><?php echo esc_html( $subtitle_text ); ?></span>
                    <?php endif; ?>
                </span>
            </div>
            <div class="gvn-corporate__secure">
    ...
    <form name="checkout" method="post" class="checkout woocommerce-checkout gvn-form gvn-corporate__form" action="<?php echo esc_url( function_exists( 'wc_get_checkout_url' ) ? wc_get_checkout_url() : '' ); ?>" enctype="multipart/form-data">

        <div class="gvn-corporate__grid">

            <!-- Coluna principal: dados do cliente (DOM primeiro = mesma ordem de hooks do clássico) -->
            <div class="gvn-corporate__main">
                <?php do_action( 'woocommerce_checkout_before_customer_details' ); ?>

                <section class="gvn-corporate__section" id="customer_details" aria-labelledby="gvn-corporate-participant">
                    <div class="gvn-corporate__section-head">
                        <h1 class="gvn-corporate__section-title" id="gvn-corporate-participant"><?php echo esc_html( $corporate_heading ); ?></h1>
                    </div>
                        ...
                        <?php foreach ( $gvn_field_groups as $gvn_group ) : ?>
                            <?php if ( ! empty( $gvn_group['fields'] ) ) : ?>
                                ...
                                <?php foreach ( $gvn_group['fields'] as $gvn_field ) : ?>
                                    <?php include __DIR__ . '/partials/field.php'; ?>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        <?php endforeach; ?>
                    </div>
            ...
            </div><!-- .gvn-corporate__main -->

            <!-- Coluna lateral: resumo do pedido -->
            <aside class="gvn-corporate__summary">
                <?php do_action( 'woocommerce_checkout_before_order_review_heading' ); ?>

                <div class="gvn-corporate__panel">
                    <h2 class="gvn-corporate__panel-title"><?php esc_html_e( 'Resumo do pedido', 'gvn-checkout' ); ?></h2>
                    ...
                    <?php if ( $coupon_enabled ) : ?>
                        <div class="gvn-corporate__coupon">
                            <button type="button" class="gvn-corporate__coupon-link" id="gvn-coupon-toggle" aria-expanded="false" aria-controls="gvn-coupon-form">
                                <?php esc_html_e( 'Adicionar cupom', 'gvn-checkout' ); ?>
                            </button>
                    ...
                    <div class="gvn-submit gvn-corporate__submit">
                        <?php wp_nonce_field( 'woocommerce-process_checkout', 'woocommerce-process-checkout-nonce' ); ?>
                        <?php
                        $order_button_text = apply_filters( 'woocommerce_order_button_text', $button_text );
                        $order_button_html = '<button type="submit" class="gvn-submit__btn gvn-corporate__cta" name="woocommerce_checkout_place_order" id="place_order" value="' . esc_attr( $order_button_text ) . '">' . esc_html( $order_button_text ) . '</button>';
                    ...
                    <div class="gvn-corporate__trust">
                        <span class="gvn-corporate__trust-item">
                            <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                            <?php esc_html_e( 'Pagamento seguro', 'gvn-checkout' ); ?>
                        </span>
                        <span class="gvn-corporate__trust-sep" aria-hidden="true">•</span>
                        <span class="gvn-corporate__trust-item">
                            ...
                            <?php esc_html_e( 'Garantia de 7 dias', 'gvn-checkout' ); ?>
                        </span>
                    </div>
                </div>

                <?php do_action( 'woocommerce_checkout_after_order_review' ); ?>
            </aside><!-- .gvn-corporate__summary -->

            <!-- Coluna principal (continuação): pagamento -->
            <div class="gvn-corporate__pay">
                <section class="gvn-corporate__section" aria-labelledby="gvn-corporate-payment">
                    <div class="gvn-corporate__section-head">
                        <h2 class="gvn-corporate__section-title" id="gvn-corporate-payment"><?php esc_html_e( 'Forma de pagamento', 'gvn-checkout' ); ?></h2>
                    </div>
                    ...
                    <div class="gvn-gateways" id="payment">
                        <?php if ( ! empty( $available_gateways ) ) : ?>
                            <div class="gvn-gateways__list gvn-corporate__tabs" role="tablist">
                                <?php
                                $first = true;
                                foreach ( $available_gateways as $gateway_id => $gateway ) :
                                ?>
                                    <div class="gvn-gateway <?php echo $first ? 'gvn-gateway--active' : ''; ?>" data-gateway="<?php echo esc_attr( $gateway_id ); ?>" role="tab" aria-selected="<?php echo $first ? 'true' : 'false'; ?>">
                    ...
                    ?>
                        <div class="gvn-order-bump gvn-corporate__bump" id="gvn-order-bump">
            ...
            </div><!-- .gvn-corporate__pay -->

        </div><!-- .gvn-corporate__grid -->

    </form>

    <footer class="gvn-corporate__footer">
        <div class="gvn-corporate__footer-inner">
            <span class="gvn-corporate__footer-brand"><?php echo esc_html( $header_text ); ?></span>
            <nav class="gvn-corporate__footer-links" aria-label="<?php esc_attr_e( 'Institucional', 'gvn-checkout' ); ?>">
                <?php if ( '' !== $terms_url ) : ?>
                    <a href="<?php echo esc_url( $terms_url ); ?>"><?php esc_html_e( 'Termos de Uso', 'gvn-checkout' ); ?></a>
                <?php endif; ?>
                <?php if ( '' !== $privacy_url ) : ?>
                    <a href="<?php echo esc_url( $privacy_url ); ?>"><?php esc_html_e( 'Privacidade', 'gvn-checkout' ); ?></a>
                <?php endif; ?>
            </nav>
        </div>
    </footer>
...
</div><!-- .gvn-checkout.gvn-layout-corporate -->
