<?php
/**
 * Layout "Minimalista" do checkout GVN ( SaaS claro: seções numeradas + resumo sticky ).
 *
 * Modelo selecionável via Configurações > GVN Checkout > Layout
 * ou pelo shortcode [gvn-checkout layout="minimal"].
 *
...
$minimal_heading = '' !== trim( (string) $title_text ) ? $title_text : __( 'Seus dados', 'gvn-checkout' );
?>

<div class="gvn-checkout gvn-layout-minimal" id="gvn-checkout" data-layout="minimal">

    <?php do_action( 'woocommerce_before_checkout_form', $checkout ); ?>

    <header class="gvn-minimal__header">
        <div class="gvn-minimal__header-inner">
            <div class="gvn-minimal__brand">
                <span class="gvn-minimal__logo" aria-hidden="true">
                    <svg width="18" height="18" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"></polyline></svg>
                </span>
                <span class="gvn-minimal__brand-name"><?php echo esc_html( $header_text ); ?></span>
            </div>
            <div class="gvn-minimal__secure">
    ...
    <form name="checkout" method="post" class="checkout woocommerce-checkout gvn-form gvn-minimal__form" action="<?php echo esc_url( function_exists( 'wc_get_checkout_url' ) ? wc_get_checkout_url() : '' ); ?>" enctype="multipart/form-data">

        <div class="gvn-minimal__grid">

            <!-- Coluna principal: dados do cliente (DOM primeiro = mesma ordem de hooks do clássico) -->
            <div class="gvn-minimal__main">
                <?php do_action( 'woocommerce_checkout_before_customer_details' ); ?>

                <section class="gvn-minimal__card" id="customer_details">
                    <h2 class="gvn-minimal__card-title"><span class="gvn-minimal__step">1.</span> <?php echo esc_html( $minimal_heading ); ?></h2>
                    <?php if ( '' !== trim( (string) $subtitle_text ) ) : ?>
                        <p class="gvn-minimal__card-sub"><?php echo esc_html( $subtitle_text ); ?></p>
                    <?php endif; ?>
            ...
            </div><!-- .gvn-minimal__main -->

            <!-- Coluna lateral: resumo do pedido -->
            <aside class="gvn-minimal__summary">
                <?php do_action( 'woocommerce_checkout_before_order_review_heading' ); ?>

                <div class="gvn-minimal__card">
                    <h3 class="gvn-minimal__card-title"><?php esc_html_e( 'Resumo do pedido', 'gvn-checkout' ); ?></h3>
                    ...
                    <?php if ( $coupon_enabled ) : ?>
                        <div class="gvn-minimal__coupon">
                            <button type="button" class="gvn-minimal__coupon-link" id="gvn-coupon-toggle" aria-expanded="false" aria-controls="gvn-coupon-form">
                                <?php esc_html_e( 'Possui cupom?', 'gvn-checkout' ); ?>
                            </button>
                    ...
                    <div class="gvn-submit gvn-minimal__submit">
                        <?php wp_nonce_field( 'woocommerce-process_checkout', 'woocommerce-process-checkout-nonce' ); ?>
                        <?php
                        $order_button_text = apply_filters( 'woocommerce_order_button_text', $button_text );
                        $order_button_html = '<button type="submit" class="gvn-submit__btn gvn-minimal__cta" name="woocommerce_checkout_place_order" id="place_order" value="' . esc_attr( $order_button_text ) . '">' . esc_html( $order_button_text ) . '</button>';
                    ...
                    <p class="gvn-minimal__trust">
                        <svg width="13" height="13" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></path></svg>
                        <span><?php esc_html_e( 'Pagamento seguro', 'gvn-checkout' ); ?> &nbsp;•&nbsp; <?php esc_html_e( 'Garantia de 7 dias', 'gvn-checkout' ); ?></span>
                    </p>
                </div>

                <?php do_action( 'woocommerce_checkout_after_order_review' ); ?>
            </aside><!-- .gvn-minimal__summary -->

            <!-- Coluna principal (continuação): pagamento -->
            <div class="gvn-minimal__pay">
                <section class="gvn-minimal__card">
                    <h2 class="gvn-minimal__card-title"><span class="gvn-minimal__step">2.</span> <?php esc_html_e( 'Pagamento', 'gvn-checkout' ); ?></h2>

                    <?php do_action( 'woocommerce_review_order_before_payment' ); ?>

                    <div class="gvn-gateways" id="payment">
                        <?php if ( ! empty( $available_gateways ) ) : ?>
                            <div class="gvn-gateways__list gvn-minimal__tabs" role="tablist">
                                <?php
                                $first = true;
                    ...
                    ?>
                        <div class="gvn-order-bump gvn-minimal__bump" id="gvn-order-bump">
            ...
            </div><!-- .gvn-minimal__pay -->

        </div><!-- .gvn-minimal__grid -->

    </form>

    <?php do_action( 'woocommerce_after_checkout_form', $checkout ); ?>

</div><!-- .gvn-checkout.gvn-layout-minimal -->
